...
label_dict = {0:'eye',1:'apple', 2:'radio', 3:'tree', 4:'bow',
              5:'basketball',6:'cloud', 7:'sun'}
...
def view_classify(img, preds):
    ...
    ax2.set_yticklabels(['eye', 'apple','radio','tree','bow','basketball', 'cloud', 'sun'], size='small');
...
def index():
    # render web page
    draw_collections = ['eye', 'apple','radio','tree','bow','basketball', 'cloud', 'sun']
    draw_item = random.choice(draw_collections)
    encoded_text = cryptocode.encrypt(draw_item,"supersecretpassword123")
    return render_template(
        'index.html',
        draw_item=draw_item.upper(),
        encoded_text=urllib.parse.quote(encoded_text)
    )

@app.route('/verify/<dataURL>')
def pred(dataURL):
    """
    ...
    decoded_text = cryptocode.decrypt(ect,"supersecretpassword123")
